...
if opt.model_name == 'MAPK':
	trainset_fn = "../data/"+opt.model_name+"/"+opt.model_name+f"_train_set_H={opt.traj_len}_2000x50.pickle"
	testset_fn = "../data/"+opt.model_name+"/"+opt.model_name+f"_test_set_H={opt.traj_len}_25x1000.pickle"
	validset_fn = "../data/"+opt.model_name+"/"+opt.model_name+f"_valid_set_H={opt.traj_len}_200x100.pickle" # to train the err predict
	activeset_fn = "../data/"+opt.model_name+"/"+opt.model_name+f"_active_set_H={opt.traj_len}_2000x50.pickle" # pool to actively query uncertain points
else:
...
Xactive = ds.XHMIN+(ds.XHMAX-ds.XHMIN)*(Xactive_scaled+1)/2
Yactive = Yactive_scaled.copy()
Yactive[:,0,opt.y_dim:] = ds.XHMIN+(ds.XHMAX-ds.XHMIN)*(Yactive_scaled[:,0,opt.y_dim:]+1)/2
Yactive[:,0,:opt.y_dim] = ds.PHMIN+(ds.PHMAX-ds.PHMIN)*(Yactive_scaled[:,0,:opt.y_dim]+1)/2
...
act_filename = f'../data/{opt.model_name}/{opt.model_name}_wgan{opt.loading_id}_{int(THRESHOLD*100)}perc_retrain_set_H={opt.traj_len}_{len(Xactive)//opt.ntrajs}x{opt.ntrajs}.pickle'
