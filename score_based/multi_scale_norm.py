...
def simulate(tend, s, r, m, sim):
	...
	if sim == 'csdi':
		...
		foldername = f"./save/EColi/ID_100/"
	...
	for c in tqdm(range(n_sim_steps)):
		...
		if sim == 'csdi':
			ini = time.time()
			s_scaled = -1+2*(s-dataset.min)/(dataset.max-dataset.min)   
			...
			'''
			NN = 1
			ctr = torch.zeros((NN,35,3))
			for kk in range(NN):
				ctr[kk,0] = torch.tensor([par[0],par[0],par[0]])
				ctr[kk,1] = torch.tensor([par[1],par[1],par[1]])
				ctr[kk,2] = torch.tensor([s_scaled[0],s_scaled[1],s_scaled[2]])
			'''
			...
			s = np.round(dataset.min+(s_scaled+1)*(dataset.max-dataset.min)/2)     
