...
if opt.active_flag:
    ...
    train_path = '../data/'+opt.model_name+'/'+opt.model_name+f'_train_set_H={opt.traj_len}_2000x10.pickle'
    ...
    new_train_path = '../data/'+opt.model_name+'/'+opt.model_name+f'_train_trajs_H={opt.traj_len+1}_2000x10.pickle'
    with open(new_train_path, "wb") as f:
        pickle.dump(new_train_dict, f)

    #------------ VALID

    valid_path = '../data/'+opt.model_name+'/'+opt.model_name+f'_valid_set_H={opt.traj_len}_200x50.pickle'
    ...
    valid_cat_trajs = np.concatenate((valid_dict['init'], valid_dict['trajs']), axis=1)

    new_valid_dict = {'trajs': valid_cat_trajs, 'n_init_states': 200, 'n_trajs_per_state':50}

    new_valid_path = '../data/'+opt.model_name+'/'+opt.model_name+f'_valid_trajs_H={opt.traj_len+1}_200x50.pickle'
    ...
    test_cat_trajs = np.concatenate((test_dict['init'], test_dict['trajs']), axis=1)

    new_test_dict = {'trajs': test_cat_trajs, 'n_init_states': 25, 'n_trajs_per_state': 1000}

    new_test_path = '../data/'+opt.model_name+'/'+opt.model_name+f'_test_trajs_H={opt.traj_len+1}_25x1000.pickle'
    with open(new_test_path, "wb") as f:
        pickle.dump(new_test_dict, f)

    # ---- ACTIVE

    active_path = '../data/'+opt.model_name+'/'+opt.model_name+f'_active_set_H={opt.traj_len}_2000x10.pickle'
    ...
    active_cat_trajs = np.concatenate((active_dict['init'], active_dict['trajs']), axis=1)

    new_active_dict = {'trajs': active_cat_trajs, 'n_init_states': 2000, 'n_trajs_per_state':10}

    new_active_path = '../data/'+opt.model_name+'/'+opt.model_name+f'_active_trajs_H={opt.traj_len+1}_2000x10.pickle'
